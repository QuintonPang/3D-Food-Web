...
const Apple = ({...props}) => {
  ...
  const height = (nodes.Apple.children[0].geometry.boundingBox.max.y-nodes.Apple.children[0].geometry.boundingBox.min.y) * scale
  const length = (nodes.Apple.children[0].geometry.boundingBox.max.x-nodes.Apple.children[0].geometry.boundingBox.min.x+0.5) * scale
  const width = (nodes.Apple.children[0].geometry.boundingBox.max.z-nodes.Apple.children[0].geometry.boundingBox.min.z) * scale
  ...
  return(
    <group position={[0,0,0]} ref={group} {...props} dispose={null}>
     ...
     <mesh position={[0,0,0]} ref={planeRef} rotation={[-Math.PI/2,0,0]}> 
        ...
        <boxGeometry attach="geometry" args ={[length*scale+10,width*scale+10,0.1*scale]}/>
