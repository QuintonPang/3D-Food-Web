...
const Burger = ({...props}) => {
  ...
  const height = (nodes.Burger.children[0].geometry.boundingBox.max.y-nodes.Burger.children[0].geometry.boundingBox.min.y ) * scale
  const length = (nodes.Burger.children[0].geometry.boundingBox.max.x-nodes.Burger.children[0].geometry.boundingBox.min.x) * scale
  const width = (nodes.Burger.children[0].geometry.boundingBox.max.z-nodes.Burger.children[0].geometry.boundingBox.min.z) * scale
  ...
  return(
    <group position={[0,0,0]} ref={group} {...props} dispose={null}>
     <mesh ref={ref} onClick={()=>{api.velocity.set(0,2*scale,0)}} position={[0,0.1*scale,0]} scale={scale} castShadow receiveShadow geometry={nodes.Burger.children[0].geometry} material={nodes.Burger.children[0].material}/>
     <mesh position={[0,0,0]} ref={planeRef} rotation={[-Math.PI/2,0,0]}> 

        {/* args is length and width and height  */}
        <boxGeometry attach="geometry" args ={[length*scale+1,width*scale+1,0.1*scale]}/>
